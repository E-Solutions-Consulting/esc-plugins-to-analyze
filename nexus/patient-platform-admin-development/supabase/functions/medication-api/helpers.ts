...
export interface MedicationEligibilityPlan {
  status?: string | null;
  expires_at?: string | null;
  cancelled_at?: string | null;
}

export function isPlanBlockingMedicationEligibility(
  plan: MedicationEligibilityPlan,
  now: number = Date.now(),
): boolean {
  if (plan.cancelled_at || plan.status === "cancelled") {
    return false;
  }
