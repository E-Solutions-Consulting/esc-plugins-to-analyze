-- Allow tenant admins to find orders by the provider's order identifier.
-- A trigram index keeps partial ID searches consistent with order-number search.
CREATE EXTENSION IF NOT EXISTS pg_trgm;

CREATE INDEX IF NOT EXISTS idx_orders_provider_platform_order_id_trgm
  ON public.orders USING gin (provider_platform_order_id gin_trgm_ops);
...
BEGIN
  ...
  candidate_orders AS (
    ...
    WHERE o.tenant_id = p_tenant_id
      ...
      AND (
        o.order_number ILIKE '%' || search_value || '%'
        OR o.provider_platform_order_id ILIKE '%' || search_value || '%'
      )
    ...
    WHERE o.tenant_id = p_tenant_id
      ...
      AND NOT (
        o.order_number ILIKE '%' || search_value || '%'
        OR COALESCE(
          o.provider_platform_order_id ILIKE '%' || search_value || '%',
          false
        )
      )
  ),
