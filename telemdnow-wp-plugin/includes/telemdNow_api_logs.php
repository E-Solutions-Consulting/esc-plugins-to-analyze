...
class TelemdNow_API_Logs {
    ...
    public function __construct() {
        global $wpdb;
        $this->table_name = $wpdb->prefix . 'telemdnow_logs';
    ...
    public  function register_plugin_menus() {
        ...
        add_submenu_page('telemdnow', 'Error Logs', 'Error Logs', 'manage_options', 'telemdnow-logs',  array($this, 'index'));
        add_submenu_page('hidden', 'View Logs', 'View Logs', 'manage_options', 'telemdnow-logs-edit', array($this, 'view'));
    }
    public function index() {

        require_once plugin_dir_path(dirname(__FILE__)) . 'templates/telemdnow-logs-listing.php';


        $table = new Telemdnow_Logs_Table();
        $table->prepare_items();
?>
        <div class="wrap">
    ...
    ?>
        <div class="wrap">
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row"><label for="data_sent">Data Sent</label></th>
                    <td><textarea name="data_sent" cols="50" rows="6" id="data_sent" class="regular-text"><?php echo esc_attr($item->data_sent); ?> </textarea></td>
                </tr>
                <tr>
                    <th scope="row"><label for="data_received">Data Received</label></th>
                    <td><textarea name="data_received" cols="50" rows="6" id="data_received" class="regular-text"><?php echo esc_attr($item->data_received); ?> </textarea></td>
                </tr>
                <tr>
