...
class Woocommerce_Webhook{
    ...
    private function find_order_id_by_telemdnow_id( $telemdnow_id ) {
        ...
        $order_id = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT order_id
                 ...
                 WHERE meta_key = 'telemdnow_entity_id'
                   AND meta_value = %s
                 LIMIT 1",
                $telemdnow_id
            )
        ...
        $order_id = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT order_id
                 FROM {$wpdb->prefix}wc_orders_meta
                 WHERE meta_key = 'telemdnow_order_id'
                   AND meta_value = %s
                 LIMIT 1",
                $telemdnow_id
            )
    ...
    function telegramdnow_update_order_status__original(WP_REST_Request $request) {
    ...
    function telegramdnow_update_order_status( WP_REST_Request $request ) {
        ...
        $woo_order_id = $this->find_order_id_by_telemdnow_id( $orderId );
        ...
        if ( ! $woo_order_id ) {
            ...
            if ( $external_identifier ) {
                $order_data = $this->retrieve_and_update_wc_order_status(
                    $external_identifier,
                    $orderStatus,
                    $orderStatusArray,
                    $eventTitle
            ...
            $request_response = 'Order not found for telemdnow_id: ' . $orderId;
            $this->telemdnow_webhook_logs( '404', $data_received, $request_response );
            return new WP_Error( 'no_orders', 'Order not found', array( 'status' => 404 ) );
        }

        $order = wc_get_order( $woo_order_id );

        if ( ! $order ) {
            $request_response = 'Could not load order ' . $woo_order_id;
            $this->telemdnow_webhook_logs( '404', $data_received, $request_response );
            return new WP_Error( 'no_orders', $request_response, array( 'status' => 404 ) );
        }
