...
class Bh_Features_Public {
	...
	*/
	function execute_tracking_and_redirect($order_id) {
		...
		// Non-UI thank-you side effects (Attentive AS, etc.) — shared with landing bridge.
		if ( function_exists( 'bh_run_purchase_side_effects' ) ) {
			bh_run_purchase_side_effects( $order, 'classic_thankyou' );
		}
	    ...
	    if(!empty($telemdnow_order_creation) && $telemdnow_order_creation !== 'false' && !empty($redirect_url)) {
		...
		$async_wait = false;
		// Classic WP thank-you never waits — async UI only for landing orders (rare on WP thank-you).
		if (
			function_exists( 'bh_is_async_telegra_landing_enabled' )
			&& bh_is_async_telegra_landing_enabled( $order )
			&& empty( $order->get_meta( 'telemdnow_visit_link', true ) )
		) {
			$async_wait = true;
		}

		$shared = function_exists( 'bh_build_purchase_tracking_payload' )
	    ...
	    $tracking_data = [
	        'redirect_url' 		=> 	$redirect_url,
	        'order_id' 			=> 	isset( $shared['order_id'] ) ? $shared['order_id'] : $order->get_id(),
	        'order_key'			=>	isset( $shared['order_key'] ) ? $shared['order_key'] : $order_key,
	        'sale_amount'		=>	isset( $shared['sale_amount'] ) ? $shared['sale_amount'] : $order->get_total(),
	        'subtotal_amount' 	=>	isset( $shared['subtotal_amount'] ) ? $shared['subtotal_amount'] : $order->get_subtotal(),
	        'email' 			=>	isset( $shared['email'] ) ? $shared['email'] : $order->get_billing_email(),
	        'discount_code' 	=>	isset( $shared['discount_code'] ) ? $shared['discount_code'] : implode( ', ', $order->get_coupon_codes() ),
	        'currency' 			=>	isset( $shared['currency'] ) ? $shared['currency'] : $order->get_currency(),
	        'async_telegra'		=>	$async_wait,
	        'status_url'		=>	rest_url( 'bh/v1/telegra-visit-status' ),
	    ];

		if ( $async_wait ) {
			echo '<div id="bh-telegra-async-wait" style="max-width:520px;margin:1.5rem auto;padding:1.25rem 1.5rem;border:1px solid #e8e4f2;border-radius:12px;background:#faf9ff;text-align:center;font-family:system-ui,sans-serif;">';
			echo '<p style="margin:0 0 0.5rem;font-size:1.1rem;font-weight:600;color:#1f0a59;">Payment successful</p>';
			echo '<p id="bh-telegra-async-msg" style="margin:0;font-size:0.95rem;color:#5c5670;line-height:1.5;">Preparing your medical questionnaire…</p>';
			echo '<p style="margin:0.75rem 0 0;font-size:0.8rem;color:#8a849c;">Please wait — do not pay again.</p>';
			echo '</div>';
		}
	}
	...
	function redirect_tracking_script_footer() {
		...
		$async = ! empty( $tracking_data['async_telegra'] );
		$has_redirect = ! empty( $tracking_data['redirect_url'] );

		if ( $async && ! $has_redirect ) {
			$status_url = isset( $tracking_data['status_url'] ) ? $tracking_data['status_url'] : '';
			$order_id   = isset( $tracking_data['order_id'] ) ? (int) $tracking_data['order_id'] : 0;
			$order_key  = isset( $tracking_data['order_key'] ) ? $tracking_data['order_key'] : '';
			?>
			<script>
			(function () {
				var statusUrl = <?php echo wp_json_encode( esc_url_raw( $status_url ) ); ?>;
				var orderId = <?php echo (int) $order_id; ?>;
				var orderKey = <?php echo wp_json_encode( (string) $order_key ); ?>;
				var msgEl = document.getElementById('bh-telegra-async-msg');
				var maxTries = 60;
				var tries = 0;
				var intervalMs = 3000;
				...
				function poll() {
					tries++;
					if (!statusUrl || !orderId || !orderKey) {
						if (msgEl) msgEl.textContent = 'Unable to check intake status. Please refresh this page.';
						return;
					}
					...
					fetch(url, { credentials: 'same-origin' })
						...
						.then(function (data) {
							...
							if (data.status === 'done' && (data.redirect_url || data.telemdnow_visit_link)) {
								window.location.href = data.redirect_url || data.telemdnow_visit_link;
								return;
							}
							if (data.status === 'failed' || tries >= maxTries) {
								if (msgEl) {
									msgEl.textContent = data.message ||
										'We are still setting up your intake. Refresh this page or check your email. Do not pay again.';
								}
								return;
							}
						...
						.catch(function () {
							if (tries >= maxTries) {
								if (msgEl) msgEl.textContent = 'We are still setting up your intake. Please refresh. Do not pay again.';
								return;
							}
							setTimeout(poll, intervalMs);
						});
