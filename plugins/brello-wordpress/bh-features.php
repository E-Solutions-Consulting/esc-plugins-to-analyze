...
/**
 * Action Scheduler worker for Telegra.
 *
 * MUST be registered in this bootstrap file (not only inside modules/loader).
 * Staging has DISABLE_WP_CRON=true so AS runs via Async Request; if the module
 * file is not loaded in that request, AS fails with "no callbacks are registered".
 *
 * @param int|array $order_id Order id.
 */
function bh_telegra_process_order_action( $order_id ) {
	$path = plugin_dir_path( __FILE__ ) . 'includes/modules/wc/orders/telegra/class-bh-telegra-async.php';
	if ( ! class_exists( 'BH_Telegra_Async', false ) && file_exists( $path ) ) {
		require_once $path;
	}
	if ( class_exists( 'BH_Telegra_Async', false ) ) {
		BH_Telegra_Async::process( $order_id );
		return;
	}
	if ( function_exists( 'wc_get_logger' ) ) {
		wc_get_logger()->error(
			'BH_Telegra_Async class missing while processing order ' . ( is_scalar( $order_id ) ? absint( $order_id ) : 0 ),
			array( 'source' => 'brello-telegra-async' )
		);
	}
}
add_action( 'bh_telegra_process_order', 'bh_telegra_process_order_action', 10, 1 );
