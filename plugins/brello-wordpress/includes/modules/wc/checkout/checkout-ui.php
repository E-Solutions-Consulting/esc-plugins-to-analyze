...
class BH_Checkout_UI {
	...
	*/
	...
	function bh_woocommerce_checkout_fields_phone_validation($fields){
		...
		// Accept the same US formats as the main phone field / mask: (XXX) XXX-XXXX
		// and digits-only. Server-side still validates 10 digits after stripping formatting.
	...
	public function render_terms_checkbox__() {
	...
	public function render_terms_checkbox() {
	    $terms_page_id = 671478;//wc_get_page_id( 'checkout-terms-conditions' );
	    $checkout_terms_url     = $terms_page_id ? get_permalink( $terms_page_id ) : '#';

	    $terms_page_id = 144100;//wc_get_page_id( 'terms-conditions' );
	    $terms_url     = $terms_page_id ? get_permalink( $terms_page_id ) : '#';

		$privacy_page_id = 144883; //wc_get_page_id( 'privacy-policy' );
		$privacy_url     = $privacy_page_id ? get_permalink( $privacy_page_id ) : '#';


		$fields	= '<div class="cfw-custom-terms" style="margin-bottom:15px;">
	        <label>
	            <input type="checkbox" name="bh_accept_checkout_terms" id="bh_accept_checkout_terms" value="on" checked />
	            ' . sprintf(
	                __( 'I have reviewed and agree to the <a href="%s" target="_blank">Checkout Terms & Conditions</a>.', 'hw-features' ),
	                esc_url( $checkout_terms_url )
	            ) . '
	        </label>
	    </div>';
	    ...
	    $fields	.= '<div class="cfw-custom-terms" style="margin-bottom:15px;">
	        <label>
	            <input type="checkbox" name="bh_accept_terms" id="bh_accept_terms" value="on" checked />
	            ' . sprintf(
	                __( 'I have read and agree to the website <a href="%s" target="_blank">Terms & Conditions</a> & <a href="%s" target="_blank">Privacy Policy</a>.', 'hw-features' ),
	                esc_url( $terms_url ),
	                esc_url( $privacy_url )
	            ) . '
	        </label>
	...
	public function bh_checkout_item_name_with_price( $product_name, $cart_item, $cart_item_key ) {
	    ...
	    $per_month   = floor( $price / $interval );
