...
if (!class_exists('WC_Subscription_Reminder_Email_7')) :

class WC_Subscription_Reminder_Email_7 extends WC_Email {
    private $resend;
    public function __construct() {
        $this->id               =   'subscription_payment_reminder';
        $this->title            =   __('Subscription Renewal Reminder – 7 Days Before', 'woocommerce');
        $this->description      =   __('Sends an email reminder 7 days prior to the subscription renewal date.', 'woocommerce');
        $this->customer_email   =   true;

        $this->template_base    =   dirname(__DIR__, 2) . '/templates/';
        $this->template_html    =   'woocommerce/emails/customer-subscription-reminder-7.php';
        ...
        $this->resend = Resend::client(RESEND_APIKEY);
    ...
    public function trigger($subscription_id) {
        ...
        if($this->object->get_meta('_reminder_email_sent_7')) {
        ...
        //$this->recipient    =   'jaime+test_final_resend@solutionswebonline.com';
        ...
        $this->object->update_meta_data('_reminder_email_sent_7', current_time('mysql'));
        $this->object->save();
        ...
        <body>
            ...
            $custom_text = '';
                    if ( $interval == 1 ) {
                        ...
                        $custom_text = BH_WEEKS_THREE_MONTH_PLAN . ' weeks';
                    }
            ?>
            ...
            <ul>
                <li><a href="<?php echo home_url('my-account') ?>">Log into your account</a> to review your current information and make updates as needed.</li>
